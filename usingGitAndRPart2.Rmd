...
There are a couple of ways of doing this, but the following method is pretty reliable. 
...
We'll continue using the `myrrepo` repository that you just created. If it is not currently open in RStudio, find the R project file (*myrrepo.rproj*) and double click it to open it. Next click the [Pull] button in the Git tab to make sure your local copy is up-to-date. This might seem unnecessary, but it is a good practice to always click and check for updates before changing anything. 
...
Clicking this will open a small window, where you can enter the name of a new branch. Add a name and click [Create]. This will now create a new branch, push it to your GitHub repository and set RStudio to work in the new repository. If you want to checkout a different branch, click on the drop down menu next to the New Branch button. 
...
Note that you can make several local commits in this way, and then push them all to GitHub on one go. This can be useful if your connection is limited in anyway. We'll add a simple R script that reads the Mauna Loa CO$_2$ file and plots it. Open a new script and add the following code (note we need to load both the data and the file containing the plot function.
