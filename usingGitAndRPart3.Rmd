...
The easiest to understand how to deal with a conflict is with an example[^2]. We'll walk through a simple example here, then you and your partner should try to recreate this using the repository that you set up in the previous step. In our example, two people are collaborating on a project; let's call them Brian and Blake. Both of them add some text to line 3 of the README.md file. Brian adds:
...
Now, he stages, adds a commit message (e.g.`resolving merge conflict`). If everything checks out, he gets the following message:
